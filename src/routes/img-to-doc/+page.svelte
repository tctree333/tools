...
<script lang="ts">
	...
	import { encodePng, fetchURL, getPerspectiveWarp, type Image } from 'image-js';
	...
	let imageUri: string = $state();
	let image: Image = $state();
	let converted: { uri: string; width: number; height: number } = $state();

	let savedImgs: { uri: string; width: number; height: number }[] = $state([]);
	...
	function loadFile() {
		...
		reader.addEventListener('load', () => {
			fetchURL(reader.result as string).then((res) => {
				image = res;
	...
	function convert() {
		const warp = getPerspectiveWarp(points.map((p) => ({ row: p[1], column: p[0] })));
		const newImage = image
			...
			.threshold({ threshold });
		return {
			uri: 'data:image/png;base64,' + encodePng(newImage).toBase64(),
	...
	function saveToPdf() {
		const docWidth = 8.5 * 72;
		const docHeight = 11 * 72;
		const doc = new jsPDF({
		...
		savedImgs.forEach(({ uri, width, height }) => {
			...
			let y = 0;
			if (width >= height) {
				newWidth = docWidth - 36;
				newHeight = height * (newWidth / width);
				x = 18;
				y = (docHeight - newHeight) / 2;
			} else {
				newHeight = docHeight - 36;
...
>
	...
	{:else}
		<label>
			...
			<input type="file" accept="image/png,image/jpeg" bind:files onchange={loadFile} />
		</label>
	{/if}
</div>
	...
	{#if savedImgs.length > 0}
		...
		<div class="flex flex-row items-center justify-center w-full gap-4 mt-8">
			<button
				...
				onclick={saveToPdf}>Save to PDF</button
			>
			<button
				type="button"
				class="px-4 py-2 border-4 border-green-600 rounded-md"
				onclick={saveToImg}>Save as image</button
			>
		</div>
	{:else}
